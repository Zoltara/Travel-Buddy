// ─────────────────────────────────────────────────────────────────────────────
// Core scoring engine
// Score = Σ(normalizedWeight_i × score_i) − complaintPenalty
// ─────────────────────────────────────────────────────────────────────────────
import type {
  RawPropertyData,
  ScoredResort,
...
function extractPros(prop: RawPropertyData, breakdown: ScoreBreakdown): [string, string, string] {
  const pros: string[] = [];
...
function extractCons(
  prop: RawPropertyData,
  prefs: SearchPreferences,
): [string, string] {
  const cons: string[] = [];

  const missedAmenities = prefs.mustHaveAmenities.filter(
    (a) => !prop.confirmedAmenities.includes(a),
  );
  if (missedAmenities.length > 0) {
    cons.push(
      `Missing requested amenities: ${missedAmenities.slice(0, 2).join(', ')}`,
    );
  }
  ...
  for (const summary of prop.complaintSummaries) {
    if (summary.mentionRate > 0.05) {
      cons.push(
        `${summary.category.charAt(0).toUpperCase() + summary.category.slice(1)} complaints in ~${Math.round(summary.mentionRate * 100)}% of reviews`,
      );
    }
...
function pickPrimaryBookingRef(platforms: PlatformRef[]): PlatformRef {
  // Prefer booking.com > expedia > agoda > tripadvisor > google-places
  const priority: PlatformRef['platform'][] = [
    'booking.com',
    'expedia',
    'agoda',
    'tripadvisor',
...
export function scoreProperty(
  ...
  const priceScore =
    prop.aggregatedPricePerNight !== null
      ? calcPriceScore(
          prop.aggregatedPricePerNight,
          prefs.budgetPerNightMin,
          prefs.budgetPerNightMax,
        )
      : 5; // neutral if unknown

  const ratingScore =
    prop.aggregatedRating !== null ? calcRatingScore(prop.aggregatedRating) : 5;
  ...
  const locationScore = calcLocationScore(
    prop.distanceFromBeach ?? prop.distanceFromCenter,
    prefs.maxDistanceFromBeach ?? prefs.maxDistanceFromCenter ?? 10,
  );

  const amenitiesScore = calcAmenitiesScore(
    prop.confirmedAmenities,
    prefs.mustHaveAmenities,
  );

  const complaintPenalty = calcComplaintPenalty(
    prop.complaintSummaries,
    prefs.avoidComplaintCategories,
  );
