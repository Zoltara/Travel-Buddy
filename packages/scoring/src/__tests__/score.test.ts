...
function makeProperty(overrides: Partial<RawPropertyData> = {}): RawPropertyData {
  return {
    name: 'Test Resort',
...
describe('calcAmenitiesScore', () => {
  it('all requested amenities present scores 10', () => {
    expect(
      calcAmenitiesScore(['beachfront', 'spa'], ['beachfront', 'spa']),
    ).toBe(10);
  });
...
describe('calcComplaintPenalty', () => {
  ...
  it('penalizes matching complaint above threshold', () => {
    const penalty = calcComplaintPenalty(
      [{ category: 'noise', mentionRate: 0.2 }],
      ['noise'],
    );
  ...
  it('does not penalize below 5% threshold', () => {
    const penalty = calcComplaintPenalty(
      [{ category: 'noise', mentionRate: 0.03 }],
      ['noise'],
    );
...
describe('applyHardFilters', () => {
  ...
  it('eliminates below-minimum rating', () => {
    const result = applyHardFilters(
      [makeProperty({ aggregatedRating: 6.0 })],
      basePrefs,
    );
  ...
  it('eliminates property over budget (strict)', () => {
    const result = applyHardFilters(
      [makeProperty({ aggregatedPricePerNight: 600 })],
      { ...basePrefs, flexibleBudget: false },
    );
    expect(result.passed).toHaveLength(0);
  });
  ...
  it('eliminates non-beachfront when beachfront is required', () => {
    const result = applyHardFilters(
      [makeProperty({ confirmedAmenities: ['spa'] })],
      basePrefs,
    );
...
describe('rankTopThree', () => {
  it('returns at most 3 results', () => {
    const props = Array.from({ length: 10 }, (_, i) =>
      makeProperty({ name: `Resort ${i}`, aggregatedRating: 7 + i * 0.1 }),
    );
  ...
  it('assigns rank 1, 2, 3 in order', () => {
    const props = Array.from({ length: 5 }, (_, i) =>
      makeProperty({ name: `Resort ${i}` }),
    );
    const results = rankTopThree(props, basePrefs);
    expect(results.map((r: { rank: number }) => r.rank)).toEqual([1, 2, 3]);
  });
