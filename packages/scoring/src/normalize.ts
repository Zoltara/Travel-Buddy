...
export function calcPriceScore(
  ...
  if (budgetMax === budgetMin) return 10;
 ...
 */
export function calcRatingScore(rating: number): number {
 ...
 * Capped at 10.
 */
export function calcTrustScore(reviewCount: number): number {
...
export function calcLocationScore(
  ...
  if (distanceKm === undefined) return 5; // neutral if unknown
  if (maxAllowedKm <= 0) return distanceKm === 0 ? 10 : 0;
 ...
 */
export function calcAmenitiesScore(
  confirmedAmenities: readonly string[],
  requestedAmenities: readonly string[],
): number {
  if (requestedAmenities.length === 0) return 10;
  const matched = requestedAmenities.filter((a) =>
    confirmedAmenities.includes(a),
  ).length;
 ...
 */
export function calcComplaintPenalty(
  complaintSummaries: ReadonlyArray<{ category: string; mentionRate: number }>,
  avoidCategories: readonly string[],
): number {
  let penalty = 0;
  for (const summary of complaintSummaries) {
    if (
      avoidCategories.includes(summary.category) &&
