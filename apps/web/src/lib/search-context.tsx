...
function reducer(state: SearchState, action: Action): SearchState {
  switch (action.type) {
    ...
    case 'SET_TYPE_FILTERS':
      return { ...state, typeFilters: { ...state.typeFilters, ...action.payload } };
    case 'SET_PRIORITIES':
...
export function SearchProvider({ children }: { children: ReactNode }) {
  ...
  function buildPreferences(): SearchPreferences | null {
    ...
    return {
      ...
      ...(location.coordinates !== undefined ? { coordinates: location.coordinates } : {}),
      checkIn: dates.checkIn,
      checkOut: dates.checkOut,
      guests: dates.guests,
      budgetPerNightMin: dates.budgetPerNightMin,
